...
void x64_vm_init(void)
{
	...
	/**
	 * 以下设置虚拟内存，主要映射了四个区域：
	 * 1.第一个是 [UPAGES, UPAGES + size of pages[])映射到页表存储的物理地址 [pages, pages + size of pages[]) 0xff000=1020KB (不映射)
	 *   这里的PTSIZE代表页式内存管理所占用的空间(不包括4级页表)
	 * 
	 * 2.第二个是 [UENVS, UENVS+env_size)映射到envs数组存储的物理地址 [envs, envs+env_size) 0x48000=120KB
	 *   这里的PTSIZE代表页式内存管理所占用的空间(不包括4级页表)
	 * 
	 * 3.第三个是 [KSTACKTOP-KSTKSIZE, KSTACKTOP) 映射到[bootstack,bootstack+64KB) 0x10000=
...
/**
 * 初始化 pages[]中的每一项，建立 page_free_list 链表
 * 注意，完成此操作后，无需再使用 boot_alloc()
 * 仅使用页分配器 page_alloc()、page_free() 来分配和释放由 page_free_list 管理的物理内存
 * 空闲页链表:
 * page_free_list
 *     👇                ->
 * +-----------------+ 		+-----------------+
...
void page_init(void)
{
	...
	// I/O 映射 + AlvOS 内核 + DWARF debug + pml4 + pages[] + envs[].
	// [IOPHYSMEM, EXTPHYSMEM): 存放 IO 映射所需要的空间，比如 VGA 的一部分显存直接映射这个地址[0xb800, 0xb8fa]
	// PADDR(boot_alloc(0)): 返回 AlvOS内核 + dwarf debug info + pml4(PGSIZE) + pages[] + envs[NENV] 后的首个地址
...
mmio_map_region(physaddr_t pa, size_t size)
{
	 ...
	 * 因为是静态变量，所以它的值会在调用mmio_map_region()后保留(就像boot_alloc()中的nextfree一样，保持更新)
	 */
