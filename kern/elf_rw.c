...
/**
 * 为了在内核中使用 DWARF 调试信息
 * 读取内核的 ELF 各程序段的头文件信息，并构造成 DWARF 的调试信息(Dwarf_Section: name、data、va、size)
 * 存储在内核后的首个空闲地址
 */
uint64_t
...
// 从内核读取"count"字节到物理地址"pa"的"offset"，可能复制的比要求的多
void readseg(uint64_t pa, uint64_t count, uint64_t offset, uint64_t *kvoffset)
{
	uint64_t end_pa;
