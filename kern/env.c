...
// 加载全局描述符 GDT，并且初始化段寄存器gs, fs, es, ds, ss, cs
...
env_setup_vm(struct Env *e)
{
	...
	/**
	 * 由于每个用户环境都需要共享内核空间，所以对于用户环境而言，在 UTOP 以上的虚拟地址空间对于所有环境来说都是相同的，用户环境无法访问和使用这一部分虚拟地址空间
	 * 因此在初始化 env_pml4e 的时候，只需要把共享部分的复制进用户环境的地址空间就实现了页式映射的共享
	 * 不需要映射页表页的原因是，用户环境可以和内核共用这些页表页
	 * 
	 * 初始化新环境虚拟地址的内核部分：把 UTOP 以上的内容从 boot_pml4e 中复制到 env_pml4e 中
	 * 注意，boot_pml4e 是用户可读的
	 */
	size_t pml4e_map = 0;
	...
	// 安全设置：配置新环境4级页表的 UVPT 映射 env 自己的页表为只读，无法被用户篡改
	// 权限: 内核 R-, 用户 R-
...
/**
 * 向 env_free_list(进程表)申请空闲的进程，对新申请的进程的struct Env(进程描述符)进行初始化，主要是对段寄存器的初始化
 * 成功后，新环境存储在 *newenv_store
 * 成功时返回0，失败时返回<0
 * 
 * 内核要开始的第一个用户进程不是通过中断等方法来进入到内核的，而是由内核直接载入的
 * 对进程的进程描述符进行初始化，是为了模仿int x指令的作用，模拟第一个进程是通过中断进入了内核，在内核处理完了相应的操作之后，才返回用户态的
 */
...
region_alloc(struct Env *e, void *va, size_t len)
{
	...
	for (; start < end; start += PGSIZE)
	{
		...
		if (!pp)
		{
			cprintf("No memory available for allocation to environment");
			return;
		}

		// 注意，根据 env_pml4e 4级页表来控制在用户环境的虚拟地址空间分配
		// 修改 env_pml4e，建立线性地址 start 与物理页pp的映射关系，插入页表页项
		result = page_insert(e->env_pml4e, pp, (void *)start, PTE_U | PTE_P | PTE_W);
		if (result < 0)
		{
			cprintf("Cannot allocate any memory for page directory allocation");
 ...
 * 函数功能：解析ELF二进制镜像，加载其内容到新环境的用户地址空间中
 * - 每个用户环境都是一个ELF映像 像bootloader所做的那样，从ELF文件加载用户环境的初始代码区、栈和处理器标识位
 * - 这个函数仅在内核初始化期间、第一个用户态环境运行前被调用
 * - 函数将ELF镜像中所有可加载的段载入到用户地址空间中，设置e->env_tf.tf_eip为ELF文件头指示的入口（虚拟地址），以便它之后能从这里开始执行程序
 * - 清零bss段
 * - 映射程序的初始栈到物理页帧
 * 
 * 由于还没有实现文件系统，所以用户进程实际的存放的位置实际上是在内存中的，文件载入内存，实际上是内存之间的数据的复制而已
 * 所以这个函数的作用就是将嵌入在内核中，用户环境取出释放到相应链接器指定的用户虚拟地址空间里
...
void load_icode(struct Env *e, uint8_t *binary)
{
	 ...
	 * 可是，在进入 load_icode() 时，是内核态进入的，所以虚拟地址空间还是内核的空间。我们要如何对用户的虚拟空间进行操作呢？
	 * 看到:lcr3 (e->env_cr3);
	 * 这个语句在我们进入每个程序头进行具体设置时，将页表切换到用户虚拟地址空间
	 * 这样我们就可以方便的在后面使用memset()和memmove()函数对一个虚拟地址进行相应的操作了
	 * 其中e->env_cr3的值是在前面的env_setup_vm() 设置好的
	...
	// 思考：如果可以将内核数据直接复制到ELF二进制文件中存储的虚拟地址中，加载程序段就会简单得多
	...
	for (; ph < eph; ph++)
	{
		...
		if (ph->p_type == ELF_PROG_LOAD)
		{
			...
			// AlvOS分配用户空间不是连续的，而是根据ph->p_va作为每次的开始地址，以p_memsz为长度进行页分配
	...
	// 再在虚拟地址(USTACKTOP-PGSIZE)为程序映射一个物理页大小的的初始栈
