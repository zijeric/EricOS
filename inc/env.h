 ...
 * +------------------------------------+------------------+
 *                                       \--- ENVX(eid) --/
 * 环境索引 ENVX(eid) 是 procs[] 数组中的环境索引，可以调用来获取任意环境
...
struct Env
{
	 ...
	 * 最后10位是环境的索引号，要用procs索引环境管理结构 Env 就要用 ENVX(proc_id)
	 * 环境恢复运行依赖于 proc_id.
	 */
	envid_t proc_id;

	// 创建 当前环境 的环境的proc_id，通过该方式构建一个环境树，用于安全方面的决策
	...
	// IPC
	// 当环境使用 sys_ipc_recv() 等待信息时，会将这个成员置为1，然后阻塞等待；
	// 当一个环境向它发消息解除阻塞后，发送环境将此成员修改为0
	bool env_ipc_recving;

	// 如果环境要接受消息，并且是传送页，则该地址 <= UTOP
	void *env_ipc_dstva;

	// 若等待消息的环境接受到了消息，发送方将接受方此成员置为消息值
